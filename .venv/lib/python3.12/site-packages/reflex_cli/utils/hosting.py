...
class ScaleType(str):
    ...
    def __new__(cls, value: str):
        """Create a new type over str.

        Args:
            value: The value of the scale type.

        Returns:
            The created scale type.

        Raises:
            ScaleTypeError: If the value is not a valid scale type.
        """
        if value not in (cls.SIZE, cls.REGION):
            raise ScaleTypeError(f"Invalid scale type: {value}")
        return super().__new__(cls, value)

    def __repr__(self):
        """Return the representation of the scale type.

        Returns:
            str: The representation of the scale type.
        """
        return f"ScaleType({super().__repr__()})"
...
class ScaleParams:
    ...
    def set_type_from_cli_args(self, cli_args: ScaleAppCliArgs) -> ScaleParams:
        ...
        if not cli_args.is_valid:
            if str(scale_type) == ScaleType.SIZE and not cli_args.vm_type:
                raise ScaleParamError(
                    f"'vmtype' should be provided in the {constants.Dirs.CLOUD} for size scaling"
                )

            if str(scale_type) == ScaleType.REGION and not cli_args.regions:
                raise ScaleParamError(
...
def get_authenticated_client(
    ...
    Args:
        token (str): The authentication token.
        interactive (bool): If running in interactive mode.
...
class SilentBackgroundBrowser(webbrowser.BackgroundBrowser):
    ...
    def open(self, url, new=0, autoraise=True):
        ...
        Args:
            ...
            new (int): Whether to open in a new window (2), tab (1), or the same tab (0).
...
def search_app(
    ...
    Args:
        ...
        interactive (bool): Whether to interactively resolve conflicts.
...
def search_project(
    ...
    Args:
        project_name (str): The name of the application to search for.
        client: The authenticated client
        interactive (bool): Whether to interactively resolve conflicts.
...
def get_project_usage(project_id: str, client: AuthenticatedClient):
    ...
    if not isinstance(client, AuthenticatedClient):
        raise NotAuthenticatedError("not authenticated")
    response = httpx.get(
        urljoin(constants.Hosting.HOSTING_SERVICE, f"/v1/project/{project_id}/usage"),
        headers=authorization_header(client.token),
        timeout=constants.Hosting.TIMEOUT,
    )
    response.raise_for_status()
...
def scale_app(app_id, scale_params: ScaleParams, client: AuthenticatedClient):
    ...
    Args:
        app_id (str): The ID of the application.
        scale_params (ScaleParams): The scaling parameters.
        client: The authenticated client
